...
HEADERS = {"User-Agent": "codex-bitget-futures-metrics/1.0"}
LOOKBACK_DAYS = 365
...
def fetch_contracts() -> list[dict]:
    ...
    return [
        ...
        if item.get("symbolStatus") == "normal" and item.get("symbolType") == "perpetual"
    ]
...
def compute_metrics(contract_info: dict) -> dict | None:
    ...
    return {
        "symbol": symbol,
        "baseCoin": contract_info.get("baseCoin", ""),
        "quoteCoin": contract_info.get("quoteCoin", ""),
        "productType": PRODUCT_TYPE,
        "symbolType": contract_info.get("symbolType", ""),
        "maxLever": contract_info.get("maxLever", ""),
        "isRwa": contract_info.get("isRwa", ""),
        "candles": len(candles),
...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_map = {executor.submit(compute_metrics, item): item["symbol"] for item in contracts}
    ...
    by_return = sorted(ranking_pool, key=lambda item: metric_key(item, "total_return_pct"), reverse=True)
    by_win_rate = sorted(ranking_pool, key=lambda item: metric_key(item, "win_rate_pct"), reverse=True)
    by_pf = sorted(ranking_pool, key=lambda item: metric_key(item, "profit_factor"), reverse=True)
    by_drawdown = sorted(ranking_pool, key=lambda item: metric_key(item, "max_drawdown_pct"), reverse=True)

    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    metrics_csv = f"bitget_usdt_futures_metrics_{timestamp}.csv"
    summary_json = f"bitget_usdt_futures_summary_{timestamp}.json"
    errors_json = f"bitget_usdt_futures_errors_{timestamp}.json"
    ...
    summary = {
        ...
        "assumptions": {
            "universe": "Bitget perpetual contracts with productType=USDT-FUTURES and symbolStatus=normal",
        ...
        "counts": {
            ...
            "ranked_contracts": len(ranking_pool),
            "errors": len(errors),
